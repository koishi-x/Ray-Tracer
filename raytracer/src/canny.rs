...
pub fn canny_check_easy(path: &str, outpath: &str) {
    ...
    for (i, j, pixel) in canny_img.enumerate_pixels_mut() {
        ...
        *pixel = image::Rgb([
            clamp(
                img.get_pixel(i, j)[0] as f64 - gradients[i as usize][j as usize],
                0.0,
                255.0,
            ) as u8,
            clamp(
                img.get_pixel(i, j)[1] as f64 - gradients[i as usize][j as usize],
                0.0,
                255.0,
            ) as u8,
            clamp(
                img.get_pixel(i, j)[2] as f64 - gradients[i as usize][j as usize],
                0.0,
                255.0,
            ) as u8,
        ]);
